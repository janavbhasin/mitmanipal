...
lower_half_pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))

labels = np.array(lower_half_pcd.cluster_dbscan(eps=0.05, min_points=5, print_progress=True))

colors = plt.cm.viridis(np.linspace(0, 1, len(np.unique(labels))))
colors = colors[labels]
lower_half_pcd.colors = o3d.utility.Vector3dVector(colors[:, :3])
